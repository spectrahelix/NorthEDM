...
export function ArtisanActions({
  ...
  async function act(action: "approve" | "reject") {
    setBusy(true);
    setErr("");
    const res = await fetch("/api/admin/artisan", {
      method: "POST",
    ...
    setDone(action === "approve" ? "Verified ✓" : "Rejected");
  }
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <Link
        href={`/profile/${userId}`}
        className="rounded-lg border border-white/10 px-3 py-1.5 text-xs text-neutral-300 transition hover:bg-white/5"
      >
        View
      </Link>
      {!verified && (
        <button
          onClick={() => act("approve")}
          disabled={busy}
          className="rounded-lg px-3 py-1.5 text-xs font-semibold text-black transition hover:opacity-90 disabled:opacity-50"
